...
def generate_obstacle_course(size, number, min_distance, ul=np.array([0, 0])):
    ...
    have = 0
    while have < number:
        new = np.around(np.random.rand(2) * size + ul)
        x_, y_ = new[0], new[1] 
        if not (x_>=-30 and x_<=30 and y_>=-65 and y_<=-5):
            print('Fatal Error!')
        is_in_scope = True
        for g in generated:
            if np.linalg.norm(new - g) <= min_distance:
...
<sdf version="1.6">
  <model name="wuao_obstacle_course">"""
...
if __name__ == '__main__':
    ...
    parser.add_argument('--a3', type=int, default=0,
                        help="Number of a3 buoys to include in challenge")
    parser.add_argument('--a5', type=int, default=0,
                        help="Number of a5 buoys to include in challenge")
    parser.add_argument('--a7', type=int, default=15,
    ...
    arr = generate_obstacle_course(size, total, margin, ul)
