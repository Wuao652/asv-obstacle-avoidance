...
class CartPoleStayUpEnv(cartpole_env.CartPoleEnv):
    def __init__(self):
        ...
        # ros_ws_abspath = "/home/dennis/simulation_ws/src"
        assert ros_ws_abspath is not None, "You forgot to set ros_ws_abspath in your yaml file of your main RL script. Set ros_ws_abspath: \'YOUR/SIM_WS/PATH\'"
        assert os.path.exists(ros_ws_abspath), "The Simulation ROS Workspace path " + ros_ws_abspath + \
                                               " DOESNT exist, execute: mkdir -p " + ros_ws_abspath + \
                                               "/src;cd " + ros_ws_abspath + ";catkin_make"
        print('Start Launch the task environment Cartpole World')
        ROSLauncher(rospackage_name="cartpole_description",
                    launch_file_name="start_world.launch",
                    ros_ws_abspath=ros_ws_abspath)
        print('\n')
        print('Launch the world model finished!')
        # Load Params from the desired Yaml file
        
        # paralist = rospy.get_param_names()
        # print('Before:')
        # print(paralist)
